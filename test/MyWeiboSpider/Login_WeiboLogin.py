...
import Login_WeiboEncode
import Login_WeiboSearch
import urllib2
import cookielib
import common
...
class WeiboLogin:
    ...
    def Login(self):
        ...
        postData = Login_WeiboEncode.PostEncode(self.userName, self.passWord, serverTime, nonce, pubkey, rsakv)#加密用户和密码
        ...
        text = result.read()
        try:
            loginUrl = Login_WeiboSearch.sRedirectData(text)#解析重定位结果
    ...
    def GetServerTime(self):
        ...
        print serverData
        try:
            serverTime, nonce, pubkey, rsakv = Login_WeiboSearch.sServerData(serverData)#解析得到serverTime，nonce等
...
def login():
    #邮箱（账号）、密码
    weiboLogin = WeiboLogin('2390635102@qq.com', '19920430')
...
def mainFunc():
    ...
    lastTime = lastLogin.read()
    lastTimeValue = int(lastTime)
    betweenTime = int(common.getCurrentTimeStamp()) - lastTimeValue
    if betweenTime > 3600 * 24:
        login()
        newLogin = open('lastLogin.txt','w')
        newLogin.write(common.getCurrentTimeStamp())
    else:
        print 'cookie 未过期'
